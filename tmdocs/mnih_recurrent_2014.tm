...
<\body>
  ...
  We want to classify images with one label. Images can vary in size but the
  number of parameters will not change. Inspiration is taken from how we
  process images, by focusing on single specific regions at a time. The
  problem is cast as a sequential decision problem, in particular a
  <abbr|POMPD>.<\footnote>
    Partially Observable Markov Decission Process. For some background on
    these <todo|we recommend ...>
  </footnote>

  Fix one image <math|x> and <math|T> timesteps.

  At each timestep <math|t=1,\<ldots\>,T>:

  <\itemize-dot>
    <item>We have a <dfn|state>: a location <math|l<rsub|t>> in the image (a
    pair of coordinates), and a corresponding \Pglimpse\Q <math|x<rsub|t>> of
    <math|x>. This glimpse is a concatenation of multiple subimages of
    <math|x> taken at different resolutions, centered at <math|l<rsub|t>>,
    then resampled to the same size. How many (<math|k>), at what resolutions
    ...
    is that it will take <em|all past state> as input, compressed into a
    hidden state vector, i.e. it will be a recurrent network. We denote its
    output at timestep <math|t> by <math|\<pi\><rsub|\<theta\>><around*|(|l<rsub|t>,a<rsub|t>\|s<rsub|1:t>|)>>,
  ...
  The goal is to maximise the total expected reward across all images. We
  detail later wrt. what the expectations are taken. The algorithm is
  basically REINFORCE<\footnote>
